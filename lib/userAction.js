...
export async function mockInterviewInviteHandler(formData, specializations) {
  try {
    ...
    let inputData = {
      ...
      interview_time: time,
      notes,
      planAmount: Number(planAmount) ? Number(planAmount) : 300,
      skills,
    };
    ...
    if (userProfileResult?.success) {
      ...
      throw new Error(
        userProfileResult?.error ||
          "Error occured While fecthing User Deatails. Please Try After Sometime"
      );
    }
  } catch (error) {
    console.log("message", error);
...
export async function mockInterviewMailUserHandler(formData, isPaid) {
  try {
    ...
    let inputData = {
      ...
      timeSlot,
      notes,
      planAmount: Number(planAmount) ? Number(planAmount) : 500,
      skills,
      paidAmount: isPaid,
