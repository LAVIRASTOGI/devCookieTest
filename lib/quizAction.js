...
export async function getQuizDetails() {
  try {
    ...
    if (quizDetails?.status === "success") {
      ...
      if (quizDetails?.statusCode === 401) {
        throw new Error("Unauthorized", {
          cause: { status: quizDetails?.statusCode },
        });
      } else {
        throw new Error(
          quizDetails?.error ||
...
export async function getQuizTopicDetails(topicId) {
  try {
    ...
    if (quizDetails?.status === "success") {
      ...
      console.log("quizDetailsssss", quizDetails);
      if (quizDetails?.statusCode === 401) {
        throw new Error("Unauthorized", {
          cause: { status: quizDetails?.statusCode },
        });
...
//subscribe to quiz
export async function subscribeQuiz(inputData) {
  try {
    const token = await getCookieValue();
    if (!token) {
      return {
  ...
  try {
    ...
    if (quizSubscription?.status === "success") {
      return {
        ...
        data:
          quizSubscription?.result && quizSubscription?.result[topicId]
            ? quizSubscription?.result[topicId]
            : {
                fullCourse: {
                  subscribed: false,
                },
      ...
      if (quizSubscription?.statusCode === 401) {
        throw new Error("Unauthorized", {
          cause: { status: quizSubscription?.statusCode },
        });
      } else {
        throw new Error(
          quizSubscription?.error ||
